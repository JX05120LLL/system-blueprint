...
const names = ['overview', 'runtime-flow', 'memory-recall', 'deployment-topology', 'before', 'after', 'traditional-microservices'];
...
try {
  ...
  await writeFile('examples/README.md', ['# 示例产物索引', '', '基础五类共六份独立模型，另附一份传统微服务架构演示。下载 HTML 后可直接离线打开；GitHub README 内的 SVG 是静态图。PNG/JPEG 使用默认 scale=2，完整展开，不受阅读器视角影响。', '', '| 示例 | 可修改数据 | 交互阅读 | 静态图 |', '| --- | --- | --- | --- |', ...rows, '', 'Before / After 同尺度合成：[SVG](../images/system-blueprint-before-after.svg) · [PNG](../images/system-blueprint-before-after.png) · [JPEG](../images/system-blueprint-before-after.jpg)。', '', '[新旧效果与桌面/窄屏/深色截图](../artifacts/visual-comparison.html) · [语义迁移说明](../docs/validation/example-migration.md) · [完整验证](../docs/validation/v2-validation-report.md)', '', '重新生成：在仓库根目录执行 `npm run examples:generate`。检查已有交付：`npm run examples:check`。', ''].join('\n'));
