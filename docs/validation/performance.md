...
2026-09-24，Windows 本机，在详情审核、动态流向与圆角多色连线升级后重新运行 `npm run test:performance`，退出码 0。完整数列、字体与环境保存在 [results.json](../../artifacts/performance/results.json)。这是指定样本在本机的测量，不是任意图形或所有设备的性能承诺。

| 指标 | 方案目标 | 最终实测 |
| --- | --- | --- |
| 30 节点 / 45 边 / 3 组，未压缩 HTML | ≤3 MiB | 1,782,775 bytes，约 1.700 MiB |
| 同一样例首次就绪，10 次 p95 | ≤1,500 ms | 261.7 ms |
| 同一样例折叠/展开，20 次 p95 | ≤500 ms | 65.4 ms |
| 100 节点 / 300 边 / 20 组首次就绪 | 完整完成，记录阻塞 | 465.7 ms；100/300/20 全部保留 |
| 上限图折叠并恢复 | 不丢原始关系 | 折叠 252.2 ms，往返 490.9 ms；恢复 100/300/20 |
| 上限图完整 SVG | 无删减或裁切 | 210.5 ms；100 节点 / 300 边，318,591 bytes |
| 上限图主线程 Long Tasks | 记录并处理阻塞 | 首次就绪无记录；折叠/导出阶段记录到一次 54 ms 任务 |
| 连续缩放/平移 | 不重跑 ELK | layoutRuns 前后均为 21；轨迹内最长 EventDispatch 为 2.33 ms |
...
多段斜线布局最初在上限样本中让一条连接穿过非端点节点，几何校验正确拒绝了该结果。后续修正了斜线端口贴合形状时的侧面判定；最终上表的上限样本直接采用多段斜线，无几何错误，**465.7 ms 不含回退**。仍保留保险逻辑：仅在节点或组标题碰撞时最多重试一次正交布局，两种路线的安全转角都可绘制小圆弧。若其他图触发回退，耗时会增加；本次样本不能量化该额外耗时。该回退也不处理连线与其他边标签的遮挡。
...
- HTML 体积主要来自内嵌 ELK Worker，单个标准文件约 1.70 MiB。压缩包体积不用于替代未压缩 HTML 指标。
