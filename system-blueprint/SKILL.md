---
name: system-flow
description: Use when users need a reviewable software-system flowchart or architecture diagram showing components, dependencies, request and data paths, decisions, or deployment boundaries from repository evidence, design documents, or a supplied system description.
---

# System Flow

为软件系统生成可审阅的流程与架构图。先由 Agent 核查仓库、文档或用户描述并建立图数据，再由本地程序校验、布局与渲染；不能把生成的关系当成自动证实的系统事实。默认交付可继续修改的 `.diagram.json` 与自包含交互 HTML；阅读者可在详情板修订已有节点、连接和分组，再下载修订文件。按需附独立 SVG、PNG、JPEG。默认浅色工程风，支持深色。用户明确要求 Mermaid 时遵循其格式。

## 路由与工作流

1. 确定读者、范围、图类型、主要阅读方向和交付格式。多分支、跨组连接或长调用链先试 RIGHT；主路径短而线性的流程先试 DOWN。生成后比较全图与阅读视角：若主路径反复折返、连接绕行、标签拥挤或缩放后文字难读，就换方向或拆成总览与子图。方向不靠节点数阈值决定。Before/After 用两份同粒度文档。合理可推断时直接推进。
...
5. 按 [视觉与验收规则](references/visual-guidelines.md) 实际打开、操作并检查截图，特别检查连线转折、颜色辨识度和横向/纵向所占空间。生成成功不等于视觉通过；文字必须先测量再布局，不能缩小字号掩盖溢出。
...
在生成的 HTML 中点选节点、连接或分组，打开 **编辑详情**。节点可改名称、摘要、说明、类型、所属分组、信息依据和来源；连接可改两端、条件/标签、说明、类型、方向、信息依据和来源；分组可改名称、说明与父分组。保存时验证整个模型并重新布局。诊断未通过时保留原图和表单；可撤销/重做已接受的修改。底栏 **横向 / 纵向** 可切换全图布局，方向选择同样可撤销/重做；下载的图数据、修订 HTML 和静态 SVG 使用所选方向。
...
- 节点以低饱和蓝、琥珀、紫、青绿、绿区分处理、判断、存储、外部、起止；普通连接按来源节点使用可辨别的多色调色板。可避障的多段路线绘成连续曲线，简单连接保持直线；不能安全平滑时保留明确的避障线段，不用短圆角掩盖绕行。异常与反馈保留语义色，关系仍用文字、箭头和线型表达。主路径有向连接可播放缓慢流向，支持暂停，并遵守系统减少动态效果偏好；这不表示实时运行。
...
- 图模型表达软件组件、关系与运行路径；没有项目排期、任务进度或甘特图语义。
